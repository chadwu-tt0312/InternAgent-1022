...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(backbone_urls[arch],
                                              progress=progress)
...
def init_deeplabv3plus(num_classes, backbone_name, output_stride, pretrained_backbone):
    if output_stride==8:
    ...
    backbone = _resnet(backbone_name, 
                       Bottleneck, 
                       [3, 4, 23, 3], # resnet50 [3, 4, 6, 3]; resnet101 [3, 4, 23, 3]
...
class ExtRandomHorizontalFlip(object):
    ...
    def __call__(self, img, lbl):
        ...
        if random.random() < self.p:
            return vF.hflip(img), vF.hflip(lbl)
        return img, lbl
...
class ExtCenterCrop(object):
    ...
    def __call__(self, img, lbl):
        ...
        """
        return vF.center_crop(img, self.size), vF.center_crop(lbl, self.size)
...
class ExtRandomScale(object):
    ...
    def __call__(self, img, lbl):
        ...
        return vF.resize(img, target_size, self.interpolation), vF.resize(lbl, target_size, Image.NEAREST)
...
class ExtRandomRotation(object):
    ...
    def __call__(self, img, lbl):
        ...
        return vF.rotate(img, angle, self.resample, self.expand, self.center), vF.rotate(lbl, angle, self.resample, self.expand, self.center)
...
class ExtRandomVerticalFlip(object):
    ...
    def __call__(self, img, lbl):
        ...
        """
        if random.random() < self.p:
            return vF.vflip(img), vF.vflip(lbl)
        return img, lbl
...
class ExtPad(object):
    ...
    def __call__(self, img, lbl):
        ...
        im = vF.pad(img, ( pw//2, pw-pw//2, ph//2, ph-ph//2) )
        lbl = vF.pad(lbl, ( pw//2, pw-pw//2, ph//2, ph-ph//2))
        return im, lbl
...
class ExtToTensor(object):
    ...
    def __call__(self, pic, lbl):
        ...
        """
        if self.normalize:
            return vF.to_tensor(pic), torch.from_numpy( np.array( lbl, dtype=self.target_type) )
        else:
...
class ExtNormalize(object):
    ...
    def __call__(self, tensor, lbl):
        ...
        """
        return vF.normalize(tensor, self.mean, self.std), lbl
...
class ExtRandomCrop(object):
    ...
    def __call__(self, img, lbl):
        ...
        if self.padding > 0:
            img = vF.pad(img, self.padding)
            lbl = vF.pad(lbl, self.padding)

        # pad the width if needed
        if self.pad_if_needed and img.size[0] < self.size[1]:
            img = vF.pad(img, padding=int((1 + self.size[1] - img.size[0]) / 2))
            lbl = vF.pad(lbl, padding=int((1 + self.size[1] - lbl.size[0]) / 2))

        # pad the height if needed
        if self.pad_if_needed and img.size[1] < self.size[0]:
            img = vF.pad(img, padding=int((1 + self.size[0] - img.size[1]) / 2))
            lbl = vF.pad(lbl, padding=int((1 + self.size[0] - lbl.size[1]) / 2))

        i, j, h, w = self.get_params(img, self.size)

        return vF.crop(img, i, j, h, w), vF.crop(lbl, i, j, h, w)
...
class ExtResize(object):
    ...
    def __call__(self, img, lbl):
        ...
        Returns:
            PIL Image: Rescaled image.
        """
        return vF.resize(img, self.size, self.interpolation), vF.resize(lbl, self.size, Image.NEAREST)
...
def main(opts):
    ...
    log_dir = os.getenv("TENSORBOARD_LOG_PATH", os.path.join(opts.out_dir, 'logs'))
    ...
    model = init_deeplabv3plus(num_classes=opts.num_classes, backbone_name='resnet101', output_stride=opts.output_stride, pretrained_backbone=True)
    ...
    interval_loss = 0
    latest_checkpoints = []
    if not os.path.exists(f'{opts.out_dir}/checkpoints'):
        os.mkdir(f'{opts.out_dir}/checkpoints')
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            if (cur_itrs) % opts.val_interval == 0:
                ckpt_path = f'{opts.out_dir}/checkpoints/latest_{cur_itrs}_{opts.model}_{opts.dataset}_os{opts.output_stride}.pth'
                save_ckpt(ckpt_path)
                ...
                if val_score['Mean IoU'] > best_score:  # save best model
                    ...
                    # Do not change dir
                    save_best_ckpt_path = os.path.join(opts.out_dir, "ckpt")
                    os.makedirs(save_best_ckpt_path, exist_ok=True)
                    save_ckpt(f"{save_best_ckpt_path}/best.pth")
                    with open(f"{save_best_ckpt_path}/best_score.txt", 'a') as f:
                        f.write(f"iter:{cur_itrs}\n{str(best_score)}\n")

                    with open(f"{opts.out_dir}/final_info.json", "w") as f:
                        final_info = {
                            "voc12_aug": {
                                "means": {
